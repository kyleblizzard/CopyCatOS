...
// AuraOS Window Manager — Asset loading
...
static cairo_surface_t *load_png(const char *path)
{
    ...
    if (cairo_surface_status(s) != CAIRO_STATUS_SUCCESS) {
        fprintf(stderr, "[aura-wm] Failed to load asset: %s (%s)\n",
                path, cairo_status_to_string(cairo_surface_status(s)));
        cairo_surface_destroy(s);
        return NULL;
    }
    fprintf(stderr, "[aura-wm] Loaded asset: %s (%dx%d)\n",
            path,
...
void assets_load(AuraWM *wm)
{
    ...
    if (s_close && s_minimize && s_zoom) {
        fprintf(stderr, "[aura-wm] All decoration assets loaded\n");
    } else {
        fprintf(stderr, "[aura-wm] WARNING: Some assets missing — using fallback rendering\n");
    }
}
