...
void frame_window(AuraWM *wm, Window client)
{
    ...
    extern void client_read_wm_class(AuraWM *wm, Client *c);
...
void unframe_window(AuraWM *wm, Client *c)
{
    ...
    if (getenv("AURA_DEBUG")) {
        fprintf(stderr, "[aura-wm] Unframed window '%s' client=0x%lx\n",
                c->title, c->client);
    }
...
void frame_existing_windows(AuraWM *wm)
{
    ...
    fprintf(stderr, "[aura-wm] Framed %d existing windows\n", wm->num_clients);
}
