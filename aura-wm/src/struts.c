...
// AuraOS Window Manager — Strut (reserved screen space) management
//
...
void struts_init(AuraWM *wm)
{
    ...
    // AuraOS uses a single desktop for now, so we advertise 1.
    long num_desktops = 1;
    ...
    fprintf(stderr, "[aura-wm] Struts initialised — workarea %dx%d+%d+%d\n",
            wa_w, wa_h, wa_x, wa_y);
}
...
// ===========================================================================
bool struts_read_strut(AuraWM *wm, Window w, long strut[12])
{
    Atom type_ret;
    int fmt;
...
// ===========================================================================
void struts_recalculate(AuraWM *wm)
{
    ...
    if (!XQueryTree(wm->dpy, wm->root, &root_ret, &parent_ret,
                    &children, &nchildren))
    {
        fprintf(stderr, "[aura-wm] struts_recalculate: XQueryTree failed\n");
        return;
    }
    ...
    fprintf(stderr, "[aura-wm] Workarea recalculated: %dx%d+%d+%d "
...
// ===========================================================================
void struts_get_workarea(AuraWM *wm, int *x, int *y, int *w, int *h)
{
...
// ===========================================================================
void struts_clamp_to_workarea(AuraWM *wm, int *x, int *y, int w, int h)
{
    (void)wm;
