...
// AuraOS Window Manager — Keyboard input (macOS-style shortcuts)
...
void input_setup_grabs(AuraWM *wm)
{
    ...
    fprintf(stderr, "[aura-wm] Key grabs set (Super+Q/W/M/H/Tab)\n");
}
...
void input_handle_key(AuraWM *wm, XKeyEvent *e)
{
    ...
    case XK_h:
        ...
        if (wm->focused) {
            extern void client_hide_app(AuraWM *wm, Client *c);
