...
//
// AuraOS Window Manager — Client window tracking and app grouping
//
...
void client_read_wm_class(AuraWM *wm, Client *c)
{
    c->wm_class[0] = '\0';
    c->wm_class_name[0] = '\0';
...
void client_hide_app(AuraWM *wm, Client *c)
{
    ...
    if (hidden > 0) {
        fprintf(stderr, "[aura-wm] Hid %d window(s) of app '%s'\n", hidden, app_class);
...
void client_smart_zoom(AuraWM *wm, Client *c)
{
    if (!c || !c->frame) return;
    ...
    if (c->zoomed) {
        ...
        extern void struts_get_workarea(AuraWM *wm, int *x, int *y, int *w, int *h);
    ...
    fprintf(stderr, "[aura-wm] Smart zoom: %s → %dx%d+%d+%d\n",
