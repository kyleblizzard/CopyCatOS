...
// AuraOS Window Manager — Edge and corner resize implementation
//
...
// ─── Module-static state ───────────────────────────────────────
// These would ideally live in the AuraWM struct, but since we
...
void resize_begin(AuraWM *wm, Client *c, int root_x, int root_y, ResizeDir dir)
{
    ...
    // (can't put it in AuraWM since we don't own that struct).
