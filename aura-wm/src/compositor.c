...
// ── Module-level state ──
// These are stored at file scope rather than in AuraWM to avoid modifying
...
bool compositor_init(AuraWM *wm)
{
    ...
    fprintf(stderr, "[aura-wm] Initializing compositor...\n");
    ...
    if (!XCompositeQueryVersion(wm->dpy, &composite_major, &composite_minor)) {
        fprintf(stderr, "[aura-wm] ERROR: XComposite extension not available\n");
        return false;
    }
    fprintf(stderr, "[aura-wm] XComposite %d.%d found\n",
    ...
    if (composite_major == 0 && composite_minor < 2) {
        fprintf(stderr, "[aura-wm] ERROR: Need XComposite >= 0.2, got %d.%d\n",
                composite_major, composite_minor);
        return false;
    }
    ...
    if (!XDamageQueryVersion(wm->dpy, &damage_major, &damage_minor)) {
        fprintf(stderr, "[aura-wm] ERROR: XDamage extension not available\n");
        return false;
    }
    fprintf(stderr, "[aura-wm] XDamage %d.%d found\n",
    ...
    if (!XFixesQueryVersion(wm->dpy, &fixes_major, &fixes_minor)) {
        fprintf(stderr, "[aura-wm] ERROR: XFixes extension not available\n");
        return false;
    }
    fprintf(stderr, "[aura-wm] XFixes %d.%d found\n", fixes_major, fixes_minor);
    ...
    if (!XRenderQueryExtension(wm->dpy, &render_event, &render_error)) {
        fprintf(stderr, "[aura-wm] ERROR: XRender extension not available\n");
        return false;
    }
    ...
    if (!compositor_create_argb_visual(wm, &argb_visual, &argb_colormap)) {
        fprintf(stderr, "[aura-wm] WARNING: No 32-bit ARGB visual found. "
                "Shadows will not be available.\n");
        // Don't fail entirely — the WM can still work without shadows
    } else {
        fprintf(stderr, "[aura-wm] Found 32-bit ARGB visual\n");
    }

    compositor_active = true;
    fprintf(stderr, "[aura-wm] Compositor initialized successfully\n");
    return true;
}
...
void compositor_set_input_shape(AuraWM *wm, Client *c)
{
    ...
    if (getenv("AURA_DEBUG")) {
        fprintf(stderr, "[aura-wm] Set input shape for '%s': "
...
void compositor_damage_notify(AuraWM *wm, XEvent *e)
{
    ...
    if (c && c->frame && c->mapped) {
        ...
        if (getenv("AURA_DEBUG")) {
            fprintf(stderr, "[aura-wm] Damage repaint for '%s'\n", c->title);
        }
    }
}
...
void compositor_shutdown(AuraWM *wm)
{
    ...
    fprintf(stderr, "[aura-wm] Compositor shut down\n");
}
