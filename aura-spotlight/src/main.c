...
// ─── main.c ───
// Entry point for aura-spotlight.
//
...
int main(void) {
    ...
    if (!dpy) {
        fprintf(stderr, "aura-spotlight: cannot open display\n");
        return 1;
    }
    ...
    if (spotlight_init(dpy) != 0) {
        fprintf(stderr, "aura-spotlight: init failed\n");
        XCloseDisplay(dpy);
        return 1;
    }

    printf("aura-spotlight: running (Ctrl+Space to toggle)\n");
