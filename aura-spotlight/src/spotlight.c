...
static void launch_entry(SearchEntry *entry) {
    ...
    if (pid == 0) {
        ...
        // so the launched app doesn't die when aura-spotlight
        // exits.
        setsid();
...
static void grab_hotkey(Display *dpy) {
    ...
    if (space == 0) {
        fprintf(stderr, "aura-spotlight: cannot find keycode for Space\n");
        return;
    }
    ...
    printf("aura-spotlight: hotkey Ctrl+Space grabbed\n");
}
...
int spotlight_init(Display *dpy) {
    ...
    if (!visual) {
        fprintf(stderr, "aura-spotlight: no 32-bit ARGB visual found, "
    ...
    XStoreName(dpy, overlay_win, "aura-spotlight");
