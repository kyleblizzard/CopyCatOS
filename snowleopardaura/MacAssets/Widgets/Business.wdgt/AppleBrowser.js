 ...
 */
function AppleBrowser(el, delegate, colWidth) {
	/** @private */
	this._div = el;
	...
	var _self = this;
	/** @private */
	this._mouseDownHandler = function() { _self._mouseDown(event); };
	/** @private */
	this._keyDownHandler = function() { _self._keyDown(event); };
	/** @private */
	this._keyUpHandler = function() { _self._keyUp(event); };
	...
	/** @private */
	this._colWidth = colWidth ? colWidth : 201;
	/** @private */
 ...
 *	@returns {Boolean}
 */
AppleBrowser._isEscKeyEvent = function(evt) { return (27 == evt.keyCode); };
...
AppleBrowser.prototype._keyDown = function(evt) {
	...
	if (AppleBrowser._isAlphaKeyEvent(evt) || AppleBrowser._isLeftArrowKeyEvent(evt) || AppleBrowser._isRightArrowKeyEvent(evt)) {
		return;
	}
	...
	try { // wrapping in try block to make sure repeating key press never runs away
		...
		} else if (AppleBrowser._isEscKeyEvent(evt)) {
			this._deselectCellInColumn(colIndex);
		}
	} catch (e) {
		shouldRepeat = false;
	}
	...
	Utils.consumeEvent(evt);
	if (shouldRepeat) {
		AppleBrowser._lastKeyDownEvent  = evt;
		AppleBrowser._keyDownTimeoutId  = setTimeout("AppleBrowser._handleKeyDownRepeat();", 150);
	}
};
 ...
 *	@returns {void}
 */
AppleBrowser._handleKeyDownRepeat = function() {
	var evt = AppleBrowser._lastKeyDownEvent;
	AppleBrowser._focusedInstance._keyDown(evt);
};
...
Utils.getTextContent = function(el) {
	...
	if (el.childNodes) {
		...
		for (var i = 0; i < el.childNodes.length; i++) {
			...
			} else if (Node.CDATA_SECTION_NODE == child.nodeType) {
				result += child.nodeValue;
			} else if (Node.ENTITY_REFERENCE_NODE == child.nodeType) {
				result += Utils.getTextContent(child);
