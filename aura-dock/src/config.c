...
// ---------------------------------------------------------------------------
// The config file lives at ~/.config/aura-dock/dock.conf
// We build this path at runtime using the HOME environment variable.
// ---------------------------------------------------------------------------
#define CONFIG_DIR_NAME   "aura-dock"
...
// ---------------------------------------------------------------------------
// config_load — Read dock configuration from ~/.config/aura-dock/dock.conf
//
...
//
// This creates/overwrites ~/.config/aura-dock/dock.conf with one line per
...
//
// The config directory is ~/.config/aura-dock/ following the XDG Base
...
// ---------------------------------------------------------------------------
// ensure_config_dir_exists — Create ~/.config/aura-dock/ if it doesn't exist.
//
// Uses mkdir() with mode 0755 (owner can read/write/execute, others can
// read/execute). We create both ~/.config and ~/.config/aura-dock if needed.
//
...
static bool ensure_config_dir_exists(void)
{
    ...
    // Now create the aura-dock config directory itself
