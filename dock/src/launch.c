...
// still a directory) or ".appdev" (developer directory). Used to decide
// whether a pinned dock item should be execed directly or handed to
// moonbase-launch so bwrap, entitlements, and the consent flow run.
// Name kept as is_appc_bundle because the metadata file it checks for
// is Info.appc, not a reference to the retired .appc bundle suffix.
