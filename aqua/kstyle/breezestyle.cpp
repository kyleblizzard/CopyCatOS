...
void Style::polish(QWidget *widget)
{
    ...
    if (qobject_cast<QScrollBar *>(widget)) {
        ...
        // For Aqua: install event filter to intercept paint and render our own scrollbar
        if (AquaAssets::instance().isValid()) {
            const_cast<QWidget*>(widget)->setAutoFillBackground(false);
            const_cast<QWidget*>(widget)->installEventFilter(const_cast<Style*>(this));
        }
...
int Style::styleHint(StyleHint hint, const QStyleOption *option, const QWidget *widget, QStyleHintReturn *returnData) const
{
    ...
    case SH_ScrollBar_Transient:
        // Force classic scrollbars so Aqua PNG assets render
        return false;
...
{
    // Aqua: intercept QScrollBar paint events to render our own scrollbar
    if (auto scrollBar = qobject_cast<QScrollBar *>(object)) {
        if (event->type() == QEvent::Paint && AquaAssets::instance().isValid()) {
            QPainter painter(scrollBar);
            const bool horizontal = (scrollBar->orientation() == Qt::Horizontal);
            const QRect fullRect = scrollBar->rect();
            const auto& aqua = AquaAssets::instance();
            const int W = fullRect.width();
            const int H = fullRect.height();

            // Build QStyleOptionSlider for sub-control rect calculations
            ...
            QRect subLine = subControlRect(CC_ScrollBar, &opt, SC_ScrollBarSubLine, scrollBar);
            QRect addLine = subControlRect(CC_ScrollBar, &opt, SC_ScrollBarAddLine, scrollBar);
            QRect slider  = subControlRect(CC_ScrollBar, &opt, SC_ScrollBarSlider, scrollBar);
            QRect groove  = subControlRect(CC_ScrollBar, &opt, SC_ScrollBarGroove, scrollBar);

            // --- Render ENTIRE scrollbar using real Snow Leopard PNG assets ---
            // Track: 3-piece stretch (top cap, center tile, bottom cap)
            if (!groove.isEmpty()) {
                const QPixmap& tTop = aqua.trackTop();
                const QPixmap& tMid = aqua.trackCenter();
                const QPixmap& tBot = aqua.trackBottom();
                if (!horizontal) {
                    int capH = tTop.height();
                    int midH = groove.height() - tTop.height() - tBot.height();
                    painter.drawPixmap(groove.x(), groove.y(), W, capH, tTop);
                    if (midH > 0)
                    ...
                    painter.drawPixmap(groove.x(), groove.y(), capW, H, lc);
                    if (midW > 0)
                        painter.drawPixmap(groove.x() + capW, groove.y(), midW, H, cc);
                    painter.drawPixmap(groove.right() - rc.width() + 1, groove.y(), rc.width(), H, rc);
                }
            }

            // Thumb: 3-piece stretch using real Aqua gel PNG assets
            if (!slider.isEmpty() && slider != groove) {
                const bool pressed = scrollBar->isSliderDown();
                const QPixmap& cTop = pressed ? aqua.thumbPressedTop()    : aqua.thumbTop();
                const QPixmap& cMid = pressed ? aqua.thumbPressedCenter() : aqua.thumbCenter();
                const QPixmap& cBot = pressed ? aqua.thumbPressedBottom() : aqua.thumbBottom();

                if (!horizontal) {
                    int capH = cTop.height();
                    int midH = slider.height() - cTop.height() - cBot.height();
                    painter.drawPixmap(slider.x(), slider.y(), W, capH, cTop);
                    if (midH > 0)
                    ...
                    painter.drawPixmap(slider.x(), slider.y(), capW, H, lc);
                    if (midW > 0)
                        painter.drawPixmap(slider.x() + capW, slider.y(), midW, H, cc);
                    painter.drawPixmap(slider.right() - rc.width() + 1, slider.y(), rc.width(), H, rc);
                }
            }

            // Arrow buttons: draw from real PNG assets
            if (!subLine.isEmpty()) {
                const QPixmap& arrow = aqua.arrowUp();
                if (!arrow.isNull())
                    painter.drawPixmap(subLine.x(), subLine.y(), subLine.width(), subLine.height(), arrow);
            }
            if (!addLine.isEmpty()) {
                const QPixmap& arrow = aqua.arrowDown();
                if (!arrow.isNull())
                    painter.drawPixmap(addLine.x(), addLine.y(), addLine.width(), addLine.height(), arrow);
            }
...
{
    // Aqua: draw text field frame from real Snow Leopard assets
    // The focused state shows the blue focus ring (classic Aqua)
    const auto& aqua = AquaAssets::instance();
    if (aqua.isValid()) {
        const bool hasFocus = option->state & State_HasFocus;
        const QRect& r = option->rect;

        // Draw a simple Aqua-style text field border
        painter->setRenderHint(QPainter::Antialiasing, true);
        painter->setPen(QPen(hasFocus ? QColor(59, 153, 252) : QColor(180, 180, 180),
                             hasFocus ? 2.0 : 1.0));
        painter->setBrush(Qt::white);
        painter->drawRoundedRect(r.adjusted(1, 1, -1, -1), 4, 4);

        // Focus glow for focused state (the classic Aqua blue ring)
        if (hasFocus) {
            painter->setPen(QPen(QColor(59, 153, 252, 80), 3.0));
...
{
    // Aqua: render push button from real Snow Leopard HITheme assets
    const auto& aqua = AquaAssets::instance();
    if (aqua.isValid() && !aqua.pushLeft().isNull()) {
        ...
        // Select the right set of 3-piece assets
        const QPixmap *left, *center, *right;
        if (down) {
            left = &aqua.pushPressedLeft(); center = &aqua.pushPressedCenter(); right = &aqua.pushPressedRight();
        } else if (isDefault) {
            left = &aqua.pushDefaultLeft(); center = &aqua.pushDefaultCenter(); right = &aqua.pushDefaultRight();
        } else {
            left = &aqua.pushLeft(); center = &aqua.pushCenter(); right = &aqua.pushRight();
        }

        // 3-piece stretch rendering
        const QRect& r = option->rect;
        int capW = left->width();
        int midW = r.width() - left->width() - right->width();
        int h = r.height();

        painter->drawPixmap(r.x(), r.y(), capW, h, *left);
        if (midW > 0)
            painter->drawPixmap(r.x() + capW, r.y(), midW, h, *center);
        painter->drawPixmap(r.right() - right->width() + 1, r.y(), right->width(), h, *right);
        return true;
    }
...
{
    // Aqua: render from real Snow Leopard HITheme checkbox assets
    const auto& aqua = AquaAssets::instance();
    if (aqua.isValid() && !aqua.checkboxOff().isNull()) {
        const State &st(option->state);
        const bool enabled(st & State_Enabled);
        const bool sunken(st & State_Sunken);
        const bool checked(st & State_On);
        const bool tristate(st & State_NoChange);
        const QPixmap* pm = &aqua.checkboxOff();
        if (!enabled) pm = &aqua.checkboxDisabled();
        else if (sunken && checked) pm = &aqua.checkboxPressedOn();
        else if (sunken) pm = &aqua.checkboxPressed();
        else if (tristate) pm = &aqua.checkboxMixed();
        else if (checked) pm = &aqua.checkboxOn();
...
{
    // Aqua: render from real Snow Leopard HITheme radio button assets
    const auto& aqua = AquaAssets::instance();
    if (aqua.isValid() && !aqua.radioOff().isNull()) {
        const State &st(option->state);
        const bool enabled(st & State_Enabled);
        const bool sunken(st & State_Sunken);
        const bool checked(st & State_On);
        const QPixmap* pm = &aqua.radioOff();
        if (!enabled) pm = &aqua.radioDisabled();
        else if (sunken && checked) pm = &aqua.radioPressedOn();
        else if (sunken) pm = &aqua.radioPressed();
        else if (checked) pm = &aqua.radioOn();
        painter->drawPixmap(option->rect, *pm);
        return true;
    }
...
bool Style::drawProgressBarContentsControl(const QStyleOption *option, QPainter *painter, const QWidget *) const
{
    ...
    // Aqua: draw progress bar fill from real Snow Leopard assets
    const auto& aqua = AquaAssets::instance();
    if (aqua.isValid() && !aqua.progressFillLeft().isNull()) {
        auto rect = option->rect;
        if (rect.width() > 0 && progressBarOption->progress > progressBarOption->minimum) {
            int capW = aqua.progressFillLeft().width();
            int midW = rect.width() - capW * 2;
            int h = rect.height();
            painter->drawPixmap(rect.x(), rect.y(), capW, h, aqua.progressFillLeft());
            if (midW > 0)
                painter->drawPixmap(rect.x() + capW, rect.y(), midW, h, aqua.progressFillCenter());
            painter->drawPixmap(rect.right() - capW + 1, rect.y(), capW, h, aqua.progressFillRight());
        }
        return true;
    }
...
{
    // Aqua: draw progress bar track from real Snow Leopard assets
    const auto& aqua = AquaAssets::instance();
    if (aqua.isValid() && !aqua.progressTrackLeft().isNull()) {
        const QRect& r = option->rect;
        int capW = aqua.progressTrackLeft().width();
        int midW = r.width() - capW * 2;
        int h = r.height();
        painter->drawPixmap(r.x(), r.y(), capW, h, aqua.progressTrackLeft());
        if (midW > 0)
            painter->drawPixmap(r.x() + capW, r.y(), midW, h, aqua.progressTrackCenter());
        painter->drawPixmap(r.right() - capW + 1, r.y(), capW, h, aqua.progressTrackRight());
        return true;
    }
...
bool Style::drawScrollBarSliderControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    // For Aqua rendering, use the full rect without offset
    // The PenWidth::Frame offset is for Breeze's thin separator line
    QRect rect = option->rect;
    const auto& aquaCheck = AquaAssets::instance();
    if (!aquaCheck.isValid()) {
        // Only apply the Breeze offset when NOT using Aqua assets
    ...
    const bool horizontal(state & State_Horizontal);
    if (aquaCheck.isValid()) {
        // Aqua: handle fills the full scrollbar width
        handleRect = rect;
    } else if (horizontal) {
    ...
    // --- Aqua scrollbar handle ---
    // When Aqua is active, the entire scrollbar (track + thumb + arrows)
    // is drawn in drawScrollBarComplexControl. Skip individual sub-control
    // rendering to prevent Breeze from overwriting our Aqua rendering.
    const auto& aqua = AquaAssets::instance();
    if (aqua.isValid()) {
        return true;  // Already drawn in drawScrollBarComplexControl
    }
    if (false && aqua.isValid()) {
        // (dead code preserved for reference)
        // Determine whether the user is pressing (dragging) the handle
        const bool handleActive(sliderOption->activeSubControls & SC_ScrollBarSlider);
        const bool pressed = handleActive && (state & State_Sunken);

        // Pick normal or pressed variants of the top/center/bottom caps
        const QPixmap& capTop    = pressed ? aqua.thumbPressedTop()    : aqua.thumbTop();
        const QPixmap& capCenter = pressed ? aqua.thumbPressedCenter() : aqua.thumbCenter();
        const QPixmap& capBottom = pressed ? aqua.thumbPressedBottom() : aqua.thumbBottom();

        // Aqua: draw directly into the full option rect
        // handleRect == rect when Aqua is active (no centering needed)
        if (horizontal) {
            QTransform rotate90;
            rotate90.rotate(90);
            const QPixmap leftCap   = capTop.transformed(rotate90);
            const QPixmap centerPx  = capCenter.transformed(rotate90);
            const QPixmap rightCap  = capBottom.transformed(rotate90);

            const int capW = leftCap.width();
            const int midW = rect.width() - leftCap.width() - rightCap.width();

            painter->drawPixmap(rect.x(), rect.y(), leftCap);
            if (midW > 0) {
                painter->drawPixmap(rect.x() + capW, rect.y(),
                                    midW, rect.height(), centerPx);
            }
            painter->drawPixmap(rect.right() - rightCap.width() + 1,
                                rect.y(), rightCap);
        } else {
            const int capH = capTop.height();
            const int midH = rect.height() - capTop.height() - capBottom.height();

            painter->drawPixmap(rect.x(), rect.y(), capTop);
            if (midH > 0) {
                painter->drawPixmap(rect.x(), rect.y() + capH,
                                    rect.width(), midH, capCenter);
            }
            painter->drawPixmap(rect.x(),
                                rect.bottom() - capBottom.height() + 1, capBottom);
        }
    } else {
        // Fallback: if assets failed to load, use original Breeze rendering
...
{
    // Aqua: already drawn in drawScrollBarComplexControl
    if (AquaAssets::instance().isValid()) return true;
    ...
    if (_addLineButtons == DoubleButton) {
        ...
        // --- Aqua: draw arrow button using real PNG asset ---
        const auto& aqua = AquaAssets::instance();
        const bool pressed = (sliderOption->activeSubControls & SC_ScrollBarAddLine) && (state & State_Sunken);
        if (aqua.isValid()) {
            const QPixmap& arrow = horizontal
                ? (pressed ? aqua.arrowDownPressed() : aqua.arrowDown())  // right arrow
                : (pressed ? aqua.arrowDownPressed() : aqua.arrowDown()); // down arrow
            if (horizontal) {
                QTransform rotate90;
                rotate90.rotate(90);
                painter->drawPixmap(rect.center() - QPoint(arrow.width()/2, arrow.height()/2),
                                    arrow.transformed(rotate90));
            } else {
                painter->drawPixmap(rect.center() - QPoint(arrow.width()/2, arrow.height()/2), arrow);
            }
        } else {
            copy.rect = rect;
            color = scrollBarArrowColor(&copy, SC_ScrollBarAddLine, widget);
...
bool Style::drawScrollBarSubLineControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (_subLineButtons == DoubleButton) {
        ...
        // --- Aqua: draw up/left arrow using PNG asset ---
        const auto& aqua = AquaAssets::instance();
        const bool arrowPressed = (sliderOption->activeSubControls & SC_ScrollBarSubLine)
                                  && (state & State_Sunken);
        if (aqua.isValid()) {
            if (horizontal) {
                // For horizontal, the "up" arrow becomes "left"
                const QPixmap& arrow = arrowPressed ? aqua.arrowUpPressed() : aqua.arrowUp();
                QTransform rotate90;
                rotate90.rotate(-90);
                const QPixmap rotated = arrow.transformed(rotate90);
                const int dx = (rect.width() - rotated.width()) / 2;
                const int dy = (rect.height() - rotated.height()) / 2;
                painter->drawPixmap(rect.x() + dx, rect.y() + dy, rotated);
            } else {
                const QPixmap& arrow = arrowPressed ? aqua.arrowUpPressed() : aqua.arrowUp();
                const int dx = (rect.width() - arrow.width()) / 2;
                const int dy = (rect.height() - arrow.height()) / 2;
                painter->drawPixmap(rect.x() + dx, rect.y() + dy, arrow);
            }
        } else {
            // Fallback to original Breeze arrows
            copy.rect = rect;
            color = scrollBarArrowColor(&copy, SC_ScrollBarSubLine, widget);
            if (horizontal) {
                if (reverseLayout) {
...
bool Style::drawTabBarTabShapeControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    // Aqua: render tab from real Snow Leopard assets
    const auto& aqua = AquaAssets::instance();
    (void)aqua; // Assets available for future use — currently drawing with QPainter
    // For tabs, use QPainter-drawn Aqua style (the tab assets need more complex handling)
    {
        ...
        painter->setRenderHint(QPainter::Antialiasing, true);
        if (selected) {
            // Active tab: white/light with blue gradient top
            QLinearGradient tabGrad(r.topLeft(), r.bottomLeft());
            tabGrad.setColorAt(0.0, QColor(180, 210, 240));  // Blue top
            tabGrad.setColorAt(0.15, QColor(220, 230, 245)); // Light blue
            tabGrad.setColorAt(1.0, QColor(255, 255, 255));  // White bottom
            painter->setBrush(tabGrad);
            painter->setPen(QPen(QColor(160, 170, 180), 1));
        } else {
            // Inactive tab: gray gradient
            QLinearGradient tabGrad(r.topLeft(), r.bottomLeft());
            tabGrad.setColorAt(0.0, QColor(210, 210, 210));
            tabGrad.setColorAt(1.0, QColor(235, 235, 235));
            painter->setBrush(tabGrad);
            painter->setPen(QPen(QColor(180, 180, 180), 1));
        }
        painter->drawRoundedRect(r.adjusted(0, 0, 0, selected ? 2 : 0), 5, 5);
        painter->setRenderHint(QPainter::Antialiasing, false);
        return true;
    }
...
{
    // Aqua: render combo box from real Snow Leopard popup button assets
    const auto& aqua = AquaAssets::instance();
    if (aqua.isValid()) {
        const bool down = option->state & QStyle::State_Sunken;
        const QRect& r = option->rect;

        // Load the 3-piece popup assets from the widget directory
        const QString wdir = QDir::homePath() + QStringLiteral("/.local/share/aqua-widgets/");
        QString prefix = down ? QStringLiteral("popup_pressed_regular")
                              : QStringLiteral("popup_active_regular");
        QPixmap left(wdir + prefix + QStringLiteral("_left.png"));
        QPixmap center(wdir + prefix + QStringLiteral("_center.png"));
        QPixmap right(wdir + prefix + QStringLiteral("_right.png"));

        if (!left.isNull() && !right.isNull()) {
            int capW = left.width();
            int rightW = right.width();
            int midW = r.width() - capW - rightW;
            int h = r.height();
            painter->drawPixmap(r.x(), r.y(), capW, h, left);
            if (midW > 0)
                painter->drawPixmap(r.x() + capW, r.y(), midW, h, center);
            painter->drawPixmap(r.right() - rightW + 1, r.y(), rightW, h, right);
            return true;
        }
    }
...
{
    // DEBUG: write a file to prove this is called
    {
        static int dbgCount = 0;
        if (dbgCount++ < 5) {
            QFile dbgF(QDir::homePath() + QStringLiteral("/.local/share/aqua-scrollbar/called.txt"));
            if (dbgF.open(QIODevice::Append | QIODevice::Text)) {
                QTextStream ts(&dbgF);
                auto so = qstyleoption_cast<const QStyleOptionSlider *>(option);
                QRect sr = so ? subControlRect(CC_ScrollBar, so, SC_ScrollBarSlider, widget) : QRect();
                QRect gr = so ? subControlRect(CC_ScrollBar, so, SC_ScrollBarGroove, widget) : QRect();
                ts << "drawSBCC valid=" << AquaAssets::instance().isValid()
                   << " rect=" << option->rect.width() << "x" << option->rect.height()
                   << " slider=" << sr.x() << "," << sr.y() << " " << sr.width() << "x" << sr.height()
                   << " groove=" << gr.x() << "," << gr.y() << " " << gr.width() << "x" << gr.height()
                   << " subControls=" << option->subControls << "\n";
                dbgF.close();
            }
        }
    }
    ...
    // --- Aqua scrollbar rendering ---
    const auto& aqua = AquaAssets::instance();
    const auto sliderOption = qstyleoption_cast<const QStyleOptionSlider *>(option);

    // Aqua scrollbar rendering note: the actual blue Aqua rendering happens
    // in the QScrollBar event filter (installed in polish()). The drawComplexControl
    // still runs but its output gets overwritten by Qt's internal painting.
    // The event filter intercepts the paint event and renders Aqua directly.

    if (aqua.isValid() && sliderOption) {
        // Get the full groove rectangle (the area between the arrow buttons)
        const QRect grooveRect = subControlRect(CC_ScrollBar, sliderOption, SC_ScrollBarGroove, widget);
        const bool horizontal = option->state & State_Horizontal;

        if (horizontal) {
            // Rotate the vertical track assets 90 degrees for horizontal scrollbars
            QTransform rotate90;
            rotate90.rotate(90);
            const QPixmap leftCap  = aqua.trackTop().transformed(rotate90);
            const QPixmap centerPx = aqua.trackCenter().transformed(rotate90);
            const QPixmap rightCap = aqua.trackBottom().transformed(rotate90);

            const int capW = leftCap.width();
            const int midW = grooveRect.width() - leftCap.width() - rightCap.width();

            painter->drawPixmap(grooveRect.x(), grooveRect.y(), leftCap);
            if (midW > 0) {
                painter->drawPixmap(grooveRect.x() + capW, grooveRect.y(),
                                    midW, grooveRect.height(), centerPx);
            }
            painter->drawPixmap(grooveRect.right() - rightCap.width() + 1,
                                grooveRect.y(), rightCap);
        } else {
            // Vertical Aqua track — light gray background with subtle inset border
            // Fill the groove with the track background color
            painter->fillRect(grooveRect, QColor(250, 250, 250));
            // Subtle inset border on left and right edges
            painter->setPen(QPen(QColor(200, 200, 200), 1));
            painter->drawLine(grooveRect.left(), grooveRect.top(),
                            grooveRect.left(), grooveRect.bottom());
            painter->drawLine(grooveRect.right(), grooveRect.top(),
                            grooveRect.right(), grooveRect.bottom());
        }
    } else {
        // Fallback: original Breeze separator line
        QRect separatorRect;
        if (option->state & State_Horizontal) {
    ...
    if (aqua.isValid() && sliderOption) {
        ...
        // --- Draw the THUMB directly here using Aqua PNG assets ---
        if (option->subControls & SC_ScrollBarSlider) {
            QRect sliderRect = subControlRect(CC_ScrollBar, sliderOption, SC_ScrollBarSlider, widget);
            const bool pressed = (sliderOption->activeSubControls & SC_ScrollBarSlider)
                                 && (option->state & State_Sunken);
            const QPixmap& capTop    = pressed ? aqua.thumbPressedTop()    : aqua.thumbTop();
            const QPixmap& capCenter = pressed ? aqua.thumbPressedCenter() : aqua.thumbCenter();
            const QPixmap& capBottom = pressed ? aqua.thumbPressedBottom() : aqua.thumbBottom();
            ...
            if (horizontal) {
                QTransform rot; rot.rotate(90);
                const QPixmap lc = capTop.transformed(rot);
                const QPixmap cc = capCenter.transformed(rot);
                const QPixmap rc = capBottom.transformed(rot);
                int capW = lc.width();
                int midW = sliderRect.width() - lc.width() - rc.width();
                painter->drawPixmap(sliderRect.x(), grooveRect.y(), lc);
                if (midW > 0) painter->drawPixmap(sliderRect.x()+capW, grooveRect.y(), midW, grooveRect.height(), cc);
                painter->drawPixmap(sliderRect.right()-rc.width()+1, grooveRect.y(), rc);
            } else {
                // Snow Leopard Aqua blue scrollbar thumb
                // Draw as a rounded blue pill matching the reference scrollbar.png
                const int thumbW = grooveRect.width();
                const int thumbH = sliderRect.height();
                const int thumbX = grooveRect.x();
                const int thumbY = sliderRect.y();
                const int radius = qMin(thumbW, thumbH) / 2;

                // Outer border (darker blue)
                painter->setRenderHint(QPainter::Antialiasing, true);
                painter->setPen(Qt::NoPen);
                painter->setBrush(QColor(100, 170, 220));
                painter->drawRoundedRect(thumbX, thumbY, thumbW, thumbH, radius, radius);

                // Inner gradient (the blue Aqua gel)
                QLinearGradient gel(thumbX, thumbY, thumbX + thumbW, thumbY);
                gel.setColorAt(0.0, QColor(180, 225, 250));   // left edge: lighter
                gel.setColorAt(0.3, QColor(147, 216, 251));   // main blue
                gel.setColorAt(0.5, QColor(170, 230, 253));   // center highlight
                gel.setColorAt(0.7, QColor(147, 216, 251));   // main blue
                gel.setColorAt(1.0, QColor(120, 195, 240));   // right edge: slightly darker
                painter->setBrush(gel);
                painter->drawRoundedRect(thumbX + 1, thumbY + 1, thumbW - 2, thumbH - 2, radius - 1, radius - 1);

                // Horizontal groove lines (the classic Aqua detail)
                if (thumbH > 20) {
                    painter->setPen(QPen(QColor(120, 190, 235, 180), 1));
                    int centerY = thumbY + thumbH / 2;
                    for (int dy = -4; dy <= 4; dy += 4) {
                        int lineY = centerY + dy;
                        if (lineY > thumbY + 4 && lineY < thumbY + thumbH - 4) {
                            painter->drawLine(thumbX + 4, lineY, thumbX + thumbW - 4, lineY);
                        }
                    }
                }
                painter->setRenderHint(QPainter::Antialiasing, false);
        ...
        // --- Draw Snow Leopard arrow buttons ---
        auto drawArrowButton = [&](const QRect& arrowRect, bool isUp) {
            // Button background — light gray gradient
            QLinearGradient bg(arrowRect.topLeft(), arrowRect.bottomLeft());
            bg.setColorAt(0.0, QColor(252, 252, 252));
            bg.setColorAt(1.0, QColor(232, 232, 232));
            painter->setBrush(bg);
            painter->setPen(QPen(QColor(180, 180, 180), 1));
            ...
            QPolygon arrow;
            if (isUp) {
                arrow << QPoint(cx, cy - 3) << QPoint(cx - 4, cy + 2) << QPoint(cx + 4, cy + 2);
            } else {
                arrow << QPoint(cx, cy + 3) << QPoint(cx - 4, cy - 2) << QPoint(cx + 4, cy - 2);
            }
            painter->setRenderHint(QPainter::Antialiasing, true);
        ...
        if ((option->subControls & SC_ScrollBarSubLine) && _subLineButtons != NoButton) {
            QRect arrowRect = subControlRect(CC_ScrollBar, sliderOption, SC_ScrollBarSubLine, widget);
            drawArrowButton(arrowRect, true);
        }
        if ((option->subControls & SC_ScrollBarAddLine) && _addLineButtons != NoButton) {
            QRect arrowRect = subControlRect(CC_ScrollBar, sliderOption, SC_ScrollBarAddLine, widget);
            drawArrowButton(arrowRect, false);
        }
    } else {
