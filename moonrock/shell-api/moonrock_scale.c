...
bool moonrock_scale_refresh(Display *dpy, MoonRockScaleTable *out)
{
    ...
    while (s < end && out->count < MOONROCK_SCALE_MAX_OUTPUTS) {
        ...
        // fields are optional: sscanf returns 6, 7, 8, or 9.
        //   6 tokens → pre-primary publisher  (primary = false)
        //   7 tokens → pre-rotation publisher (rotation = 0°)
        //   8 tokens → pre-multiplier publisher (multiplier = 1.0)
        //   9 tokens → current wire format
        // The 6th field is the *effective* scale — backing × multiplier,
        // already folded by the publisher so render code never multiplies
        // again. The 9th field is the user-chosen Interface Scale slider
        // value, carried for UI display only.
        int got = sscanf(line, "%63s %d %d %d %d %f %d %d %f",
                         namebuf, &x, &y, &w, &h, &scale,
                         &primary, &rotation, &multiplier);
        if (got >= 6 && got <= 9) {
            ...
            // Same defense-in-depth clamp on the multiplier. Publisher
            // enforces 0.5–4.0; anything outside the sane band
            // collapses to 1.0 so UI never reads a nonsense slider
            // position.
            if (got == 9 && multiplier >= 0.25f && multiplier <= 8.0f) {
                                      ...
                                      float multiplier)
{
    ...
    // Reject NaN/inf and obviously-out-of-band values. MoonRock clamps
    // to 0.5–4.0 on its side, but a 0.0 sentinel (clear override) must
    // still round-trip cleanly — treat that as the one legal "zero"
    // here. Anything else negative or absurdly large gets dropped.
    if (multiplier != multiplier) return false;                 // NaN
    if (multiplier < 0.0f || multiplier > 10.0f) return false;

    // Guard against separator characters that would break the line
    // parser on the MoonRock side. Output names are XRandR identifiers
    // (ASCII plus a few punctuation chars).
    for (const char *p = output_name; *p; p++) {
        if (*p == ' ' || *p == '\n') return false;
    }

    Atom set_atom = XInternAtom(dpy, MOONROCK_SET_INTERFACE_SCALE_ATOM_NAME, False);
    Atom utf8    = XInternAtom(dpy, "UTF8_STRING", False);

    char line[MOONROCK_SCALE_NAME_MAX + 16];
    int n = snprintf(line, sizeof(line), "%s %.3f\n",
                     output_name, (double)multiplier);
    if (n < 0 || n >= (int)sizeof(line)) return false;
